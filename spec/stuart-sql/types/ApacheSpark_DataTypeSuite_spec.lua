local FloatType = require 'stuart-sql.types.FloatType'
local IntegerType = require 'stuart-sql.types.IntegerType'
local LongType = require 'stuart-sql.types.LongType'
local registerAsserts = require 'registerAsserts'
local StringType = require 'stuart-sql.types.StringType'
local StructField = require 'stuart-sql.types.StructField'
local StructType = require 'stuart-sql.types.StructType'

registerAsserts(assert)

describe('stuart-sql/types ApacheSpark DataTypeSuite', function()

--  it('construct an ArrayType', function()
--    val array = ArrayType(StringType)
--
--    assert(ArrayType(StringType, true) === array)
--  }
--
--  it('construct an MapType', function()
--    val map = MapType(StringType, IntegerType)
--
--    assert(MapType(StringType, IntegerType, true) === map)
--  }

  it('construct with add', function()
    local struct = StructType:new()
      :add('a', IntegerType, true)
      :add('b', LongType, false)
      :add('c', StringType, true)

    assert.equal(struct['b'], StructField:new('b', LongType, false))
  end)

  it('construct with add from StructField', function()
    -- Test creation from StructField type
    local struct = StructType:new()
      :add(StructField:new('a', IntegerType, true))
      :add(StructField:new('b', LongType, false))
      :add(StructField:new('c', StringType, true))

    assert.equal(struct['b'], StructField:new('b', LongType, false))
  end)

--  it('construct with add from StructField with comments', function()
...
--  }
  ...
  it('construct with String DataType', function()
    local struct = StructType:new()
      :add('a', 'int', true)
      :add('b', 'long', false)
      :add('c', 'string', true)

    assert.equal(struct['a'], StructField:new('a', IntegerType, true))
    assert.equal(struct['b'], StructField:new('b', LongType, false))
    assert.equal(struct['c'], StructField:new('c', StringType, true))
  end)

  it('extract fields from a StructType', function()
    local struct = StructType:new()
      :add(StructField:new('a', IntegerType, true))
      :add(StructField:new('b', LongType, false))
      :add(StructField:new('c', StringType, true))
      :add(StructField:new('d', FloatType, true))
      -- :: Nil
...
--    }
  ...
  it('extract field index from a StructType', function()
    local struct = StructType:new()
      :add(StructField:new('a', LongType))
      :add(StructField:new('b', FloatType))
      -- :: Nil

    assert.equal(0, struct:fieldIndex('a'))
    assert.equal(1, struct:fieldIndex('b'))

    assert.has_error(function()
      return struct:fieldIndex('non_existent')
    end)
  end)

--  it('fieldsMap returns map of name to StructField', function()
...
--  }
--
--  it('merge where right contains type conflict', function()
--    val left = StructType(
--      StructField('a', LongType) ::
--      StructField('b', FloatType) :: Nil)
--
...
--    test(s'from Json - $dataType', function()
--      assert(DataType.fromJson(dataType.json) === dataType)
--    }
--  }
--
--  def checkDataTypeFromDDL(dataType: DataType): Unit = {
--    test(s'from DDL - $dataType', function()
...
--
--  it('fromJson throws an exception when given type string is invalid', function()
...
--    test(s'Check the default size of $dataType', function()
...
--    test(s'catalogString: $dt', function()
